...
import {
  ...
  type Transaction,
} from "@/examples/transactionhisdummy";
import { filterOptions, type FilterState } from "@/examples/filter";
  ...
  const applyFilters = (
    ...
    if (currentFilters.platforms.length > 0) {
      filtered = filtered.filter((t) =>
        currentFilters.platforms.includes(t.platform),
      );
    }
    ...
    if (currentFilters.status.length > 0) {
      filtered = filtered.filter((t) =>
        currentFilters.status.includes(t.status),
      );
    }
    ...
    if (currentFilters.dateRange.from || currentFilters.dateRange.to) {
      filtered = filtered.filter((t) => {
        ...
        const fromCheck =
          ...
          transactionDate >= currentFilters.dateRange.from;
        const toCheck =
          !currentFilters.dateRange.to ||
          transactionDate <= currentFilters.dateRange.to;
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50">
                    <TableHead className="text-left font-medium text-gray-500 uppercase tracking-wider text-xs">
                      Date
                    </TableHead>
                    <TableHead className="text-left font-medium text-gray-500 uppercase tracking-wider text-xs">
                      Platform
                    ...
                    <TableHead className="text-right font-medium text-gray-500 uppercase tracking-wider text-xs">
                      Rate
                    </TableHead>
                    <TableHead className="text-right font-medium text-gray-500 uppercase tracking-wider text-xs">
                      Charges
                    ...
                    <TableHead className="text-center font-medium text-gray-500 uppercase tracking-wider text-xs">
                      Status
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredTransactions.map((transaction) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <span
                          className={`font-medium text-sm ${getTypeColor(transaction.type)}`}
                        >
                          {transaction.type}
                        </span>
                      </TableCell>
