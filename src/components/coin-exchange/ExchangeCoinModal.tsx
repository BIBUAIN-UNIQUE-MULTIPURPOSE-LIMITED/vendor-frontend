...
export default function ExchangeCoinModal({
  ...
  const [autoMode, setAutoMode] = useState<"trade" | "timer" | "volume">("timer");
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-6">
        ...
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-semibold">Excess Coin Details – {coinSymbol}</h2>
        </div>
          ...
          <>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input disabled value={totalCoin.toString()} className="mt-1 bg-gray-100" />
              </div>
              <div>
            ...
            <div>
              ...
              <Input disabled value={excessCoin.toFixed(8)} className="mt-1 bg-gray-100" />
            </div>

            <div>
              <label className="text-sm text-gray-700 mb-2 block">Rate Option</label>
              <div className="flex items-center justify-between bg-gray-100 rounded px-4 py-2">
                <span className={rateOption === "market" ? "font-medium text-black" : "text-gray-500"}>
                  Market Rate
                </span>
                <Switch
                  checked={rateOption === "limit"}
                  onCheckedChange={(checked) => setRateOption(checked ? "limit" : "market")}
                />
                <span className={rateOption === "limit" ? "font-medium text-black" : "text-gray-500"}>
                  Limit
                </span>
              </div>
              {rateOption === "limit" && (
                <Input
                  ...
                  value={limitRate}
                  onChange={(e) => setLimitRate(parseFloat(e.target.value) || 0)}
                  className="mt-2"
                />
              )}
            </div>
        ...
        {tab === "auto" && (
          <>
            <div className="rounded bg-gray-50 p-4 border">
              <div className="text-sm text-gray-600">Total Available Excess Coin</div>
              <div className="text-xl font-semibold text-green-600">
                {totalCoin.toFixed(8)} {coinSymbol}
              </div>
              <div className="text-xs text-gray-500">≈ US ${currentRate * totalCoin}</div>
            </div>

            <div className="flex justify-between items-center">
              <span className="text-green-600 font-medium text-sm">● Auto Exchange Active</span>
              <div className="space-x-2">
                <Button size="sm" variant="outline" onClick={() => setIsAutoActive(false)} disabled={!isAutoActive}>
                  ⏸ Pause
                </Button>
              ...
              <label className="flex gap-2 items-start border p-3 rounded-md cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Trigger coin exchange whenever a new trade amount equals/exceeds excess coin
                  </p>
                </div>
              </label>
            ...
            <div>
              <label className="text-sm text-gray-700 mb-1 block">Rate Option</label>
              <div className="flex items-center justify-between bg-gray-100 rounded px-4 py-2">
                <span className={rateOption === "market" ? "font-medium text-black" : "text-gray-500"}>
                  Market Rate
                </span>
              ...
              <div className="text-sm text-gray-500 mt-1">${currentRate.toLocaleString()}</div>
            </div>
          </>
        )}
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          {tab === "manual" ? (
            <Button onClick={handleExecute} className="bg-yellow-400 hover:bg-yellow-500 text-white">
              Execute Sell Trade
            </Button>
          ) : (
            <Button disabled={!isAutoActive} className="bg-yellow-400 hover:bg-yellow-500 text-white">
              Start Auto Exchange
            </Button>
          )}
        </div>
