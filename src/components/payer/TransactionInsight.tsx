...
import { transactInsights } from "@/examples/payer/transactInsights";
    ...
    <Card className="py-3 gap-4 h-full shadow-none rounded-md">
      <CardHeader className="px-4">
        ...
        >
          ...
          <p className="text-base font-semibold">
            <span className="capitalize">transaction insight</span>
          </p>
        </Button>
      </CardHeader>
      <CardContent className="px-4">
        <table className="w-full table-auto">
          <tbody>
            {transactInsights.map((trade, index) => (
              <Fragment key={index}>
                <tr className="h-9">
                  <th className="w-7/12">
                    ...
                    <p className="text-right font-semibold">
                      <span className={`${trade.amount.color}`}>
                        {!trade.amount.currency
                          ? trade.amount.value
                          : Number(trade.amount.value).toLocaleString("en-NG", {
                              currency: "NGN",
                              style: "currency",
